# Ad Sales Prediction using Linear Regression

This project predicts product sales based on advertising expenditure across different media channels using a Linear Regression model. A simple and interactive Streamlit web app is provided to make predictions.

## Tech Stack
- Python
- Pandas
- NumPy
- Scikit-learn
- Streamlit

## Project Structure
- `advertising.csv` – Dataset used for training
- `Linear Regression AD Sales Prediction.ipynb` – Model training notebook
- `linear_regression_model.pkl` – Trained model
- `lr_app.py` – Streamlit web application
- `README.md` – Project documentation

## How to Run the Project

1. Clone the repository
git clone https://github.com/Tridha-05/ad-sales-prediction-ml.git

2. Navigate to the project folder
cd ad-sales-prediction-ml

3. Install required dependencies
pip install streamlit pandas numpy scikit-learn


4. Run the Streamlit app
streamlit run lr_app.py


## Output
The app takes advertising spend for:
- TV
- Radio
- Newspaper

and predicts the expected sales value.

## Learning Outcome
- Built a Linear Regression model
- Saved and loaded a trained ML model
- Created a Streamlit web app
- Used Git and GitHub for version control
